...
void get_tree_heights_in_row(std::vector<int>& map, std::string_view row_str) noexcept
{
    for (auto i: row_str)
        map.push_back(i - '0');
}
...
size_t find_bigger_tree_in_row(const std::vector<int>& map,
                               size_t current_row,
                               size_t current_col,
                               size_t row_size)
{
    auto elem = [&](auto row, auto col) {
        return map[row * row_size + col];
    };
    ...
    if constexpr (left)
    {
        for (; current_col >= 0 && current_col < static_cast<size_t>(-1); --current_col)
        {
...
size_t find_bigger_tree_in_col(const std::vector<int>& map,
                               size_t current_row,
                               size_t current_col,
                               size_t row_size)
{
    ...
    if constexpr (up)
    {
        for (; current_row >= 0 && current_row < static_cast<size_t>(-1); --current_row)
        {
            if (elem(current_row, current_col) >= current)
            {
                return current_row;
...
int find_visible_trees(std::vector<int>& map, size_t row_size)
{
    ...
    for (auto row = 1u; row < total_rows - 1; ++row)
    {
        ...
        {
            if (is_greatest_left(row, col) || is_greatest_right(row, col) || is_greatest_down(row, col)
                || is_greatest_up(row, col))
            {
...
void day8() noexcept
{
    ...
    std::ifstream file(home + "/Dropbox/aosc/2022/src/data/day8_input.txt");
    if (!file.is_open())
    {
