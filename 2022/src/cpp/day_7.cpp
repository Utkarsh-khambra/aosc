...
void day7() noexcept
{
    ...
    std::ifstream file(home + "/Dropbox/aosc/2022/src/data/day7_input.txt");
    if (!file.is_open())
    {
    ...
    while (std::getline(file, line))
    {
        if (line.starts_with("$"))
        {
            if (line.substr(2).starts_with("cd"))
            {
                ...
                auto itr = std::ranges::find(current->children_dir, change_dir_name, &fs_tree::dir_name);
        ...
        {
            current->children_dir.emplace_back(std::make_unique<fs_tree>(line.substr(4)));
