...
class Monkey
{
  public:
    ...
    void set_old_to_new(std::function<std::uint64_t(std::uint64_t)> f) noexcept { old_to_new.swap(f); }
    ...
    void print()
    {
        fmt::print("{} {} {} Div {}\n", item_worry, on_false->item_worry, on_true->item_worry, div);
    }
...
void day11() noexcept
{
    ...
    std::ifstream file(home + "/Dropbox/aosc/2022/src/data/day11_input.txt");
    ...
    while (std::getline(file, line))
    {
        ...
        {
            int monkey_id = std::stoi(line.substr(line.find(" ") + 1, line.find(":") - 1));
            ...
            if (line.contains("+"))
            {
                ...
                monkeys[monkey_id].set_old_to_new([=](std::uint64_t w) noexcept -> std::uint64_t {
