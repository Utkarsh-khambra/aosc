...
public:
  CharSet() : m_set(0){};
  template <std::ranges::range R> CharSet(R &&str) : m_set(0) {
    for (auto i : str) {
  ...
  [[nodiscard]] CharSet operator&(CharSet a) noexcept {
    return a.m_set & this->m_set;
  }
...
void day3() noexcept {
  ...
  while (std::getline(file, line)) {
    ...
    if (line_index > 2) {
      ...
      auto i =
          std::string(intersection).front(); // Only need just one character
      part2_sum += score(i);
      line_index = 0;
    }
