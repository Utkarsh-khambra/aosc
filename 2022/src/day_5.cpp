...
void move_part_1(std::vector<std::vector<char>> &stacks, long amount, long from,
                 long to) noexcept {
  for (auto i : std::views::iota(0, amount)) {
    stacks[static_cast<size_t>(to - 1)].push_back(
        stacks[static_cast<size_t>(from - 1)].back());
    stacks[static_cast<size_t>(from - 1)].pop_back();
  }
}

void move_part_2(std::vector<std::vector<char>> &stacks, long amount, long from,
                 long to) noexcept {
...
void day5() noexcept {
  ...
  std::ifstream file(home + "/aoc/src/data/day5_input.txt");
  if (!file.is_open()) {
  ...
  std::ranges::for_each(stacks,
                        [](auto &a) noexcept { std::ranges::reverse(a); });
  ...
  while (std::getline(file, line)) {
    ...
    for (std::string_view str(line); !str.empty();) {
      ...
      while (!str.empty() &&
             (std::isalpha(str.front()) || std::isspace(str.front())))
        str.remove_prefix(1);
      size_t count = 0;
