...
enum class Result { LOSS = 0, DRAW = 3, WIN = 6 };
enum class Move { ROCK = 1, PAPER, SCI };
...
int move_to_score(Move move) noexcept { return std::to_underlying(move); }
...
void day2() noexcept {
  ...
  while (std::getline(file, line)) {
    ...
    second_part_sum +=
        part_2(move_cast(opponent_move),
