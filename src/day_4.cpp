...
[[nodiscard]] auto parse_pair_range(std::string_view line) noexcept {
  ...
  auto elf_2_range = std::string_view(line.begin() + elf_1_range.size() + 1,
                                      line.size() - elf_1_range.size() - 1);

  auto get_range = [](std::string_view range_str) noexcept {
    auto seperator_pos = range_str.find('-');
    auto range_start =
        std::stol(std::string(range_str.substr(0, seperator_pos)));
    auto range_end =
        std::stol(std::string(range_str.substr(seperator_pos + 1)));
...
[[nodiscard]] bool has_full_overlap(std::pair<long, long> a,
                                    std::pair<long, long> b) noexcept {
...
[[nodiscard]] bool has_partial_overlap(std::pair<long, long> a,
                                       std::pair<long, long> b) noexcept {
  if (a.first == b.first || a.second == b.second)
    return true;
  if (a.first < b.first)
...
void day4() noexcept {
  ...
  std::ifstream file(home + "/aoc/src/data/day4_input.txt");
  if (!file.is_open()) {
